...
Before debugging tooling or build failures, verify the local Node runtime:

```bash
node -v
```

This project is currently validated on **Node `25.8.2`**.

- `.nvmrc` and `.node-version` are pinned to `25.8.2`
- `package.json` allows Node `>=22 <26`
- If you are on Windows and hit npm cache permission issues, use `npm install --cache .npm-cache`

If you are investigating build failures, check [next.config.ts](../next.config.ts) before changing framework versions. The `experimental.cpus = 1` and `experimental.workerThreads = true` settings are intentional and currently required for reliable Next.js builds on Windows + Node 25 in this repo.
